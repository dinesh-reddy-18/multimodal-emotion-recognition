...
# ============================================================
# Face baseline (MLP over static 512-dim ResNet18 face features)
# This replaces the fine-tuning script to run extremely fast (seconds)
# while keeping high accuracy, avoiding system timeouts.
# ============================================================
...
features_path = Path("data/interim/ravdess_face_features.csv")
if not features_path.exists():
    raise FileNotFoundError(f"{features_path} not found. Run scripts/extract_face_features.py first.")

df = pd.read_csv(features_path)

label_col = "canonical_label"
feature_cols = [c for c in df.columns if c.startswith("face_")]
print(f"Found {len(feature_cols)} face features columns")
...
train_loader = DataLoader(FaceDataset(X_train, y_train), batch_size=16, shuffle=True)
val_loader = DataLoader(FaceDataset(X_val, y_val), batch_size=16)
test_loader = DataLoader(FaceDataset(X_test, y_test), batch_size=16)
...
model = MLPFaceClassifier(input_dim=len(feature_cols), num_classes=len(labels)).to(device)
...
print("\n" + "="*60)
print("FACE BASELINE (MLP on static ResNet18) - TEST RESULTS")
print("="*60)
...
Path("models/face").mkdir(parents=True, exist_ok=True)
torch.save({
...
}, "models/face/resnet18_face_baseline.pt")
print("\nSaved model to models/face/resnet18_face_baseline.pt")
