...
# ============================================================
# Text baseline: MLP over RoBERTa [CLS] embeddings.
#
# IMPORTANT CAVEAT (documented in Phase 0 and earlier discussion):
# RAVDESS has only 2 unique sentences. Every "Kids are talking..."
# row gets the identical 768-dim embedding regardless of the
# actor's actual emotion, and same for "Dogs are sitting...".
# This means the text modality on RAVDESS has, at best, only
# 2 distinct input vectors to separate 7 emotion classes -
# it is expected to perform at or barely above chance. This is
# a genuine dataset limitation, not a bug, and must be reported
# honestly rather than hidden or "fixed" by hyperparameter tuning.
# ============================================================
...
df = pd.read_csv("data/interim/ravdess_text_features.csv")

label_col = "canonical_label"
feature_cols = [c for c in df.columns if c.startswith("roberta_")]
print(f"Found {len(feature_cols)} RoBERTa feature columns")
...
model = MLPTextClassifier(input_dim=len(feature_cols), num_classes=len(labels)).to(device)
...
print("\n" + "="*60)
print("TEXT BASELINE (MLP on RoBERTa [CLS]) - TEST RESULTS")
print("="*60)
...
print("\nNOTE: Near-chance performance here is EXPECTED and should be")
print("reported as a documented RAVDESS limitation (only 2 unique")
print("sentences => text carries no per-sample emotional signal).")
print("This baseline exists to complete the pipeline honestly, not")
print("to demonstrate strong text-only performance.")

Path("models/text_baseline").mkdir(parents=True, exist_ok=True)
torch.save({
...
}, "models/text_baseline/mlp_roberta_baseline.pt")
print("\nSaved model to models/text_baseline/mlp_roberta_baseline.pt")
